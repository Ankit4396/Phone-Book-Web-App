...
if ($mode == "add_new" ) {
  ...
  if (mysqli_num_rows($contact_no1) > 0) {
      //$name_error = "Sorry... number already exist"; 
      $error= FALSE; 

    }
    if (mysqli_num_rows($contact_no2) > 0) {
     // $name_error = "Sorry... username already exist";  
      $error= FALSE;
    }

  if (!$error) {
    $sql = "INSERT INTO `tbl_contacts` (`first_name`, `middle_name`, `last_name`, `address`, `contact_no1`, `contact_no2`, `email_address`) VALUES "
  ...
  if (!$error) {
    $sql = "UPDATE `tbl_contacts` SET `first_name` = :fname, `middle_name` = :mname, `last_name` = :lname, `address` = :address, `contact_no1` = :contact1, `contact_no2` = :contact2, `email_address` = :email,  = :pic  "
